...
/** Admin: persist a new drag-and-drop order for all service offerings. Body: { ids: string[] } in the new order. */
export async function PATCH(req: Request) {
  ...
  await prisma.$transaction(
    ids.map((id: string, index: number) =>
      prisma.serviceOffering.update({ where: { id }, data: { order: index } })
    )
  );
  ...
  await logActivity({
    ...
    entityId: 'bulk',
    entityLabel: `${ids.length} delivery lines reordered`,
